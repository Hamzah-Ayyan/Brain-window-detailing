            ...
            <AnimatePresence>
                ...
                >
                    ...
                    <div className="relative flex h-[35px] w-full items-center justify-center gap-2 border-b border-[#CBE7FF] bg-[#F0F7FF] px-4 text-sm dark:border-white/10 dark:bg-black/95 dark:backdrop-blur-md">
                        ...
                        <span className="text-[0.8125rem] text-[#005FF2] dark:text-[#44A7FF] whitespace-nowrap">
                            {description}
                        </span>
                        {onClose && (
                            <button
                                onClick={onClose}
                                className="m-0 relative overflow-hidden flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-[#005FF2] hover:bg-[#CAE7FF] dark:hover:bg-[#012F61]"
                            >
                                <GlowingEffect spread={40} glow={true} disabled={false} proximity={64} inactiveZone={0.01} borderWidth={2} />
                                <X size={16} className="text-[#005FF2] dark:text-[#47A8FF] relative z-10" />
                            </button>
                        )}
                    </div>
                </motion.div>
